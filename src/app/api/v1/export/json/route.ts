/**
 * /api/v1/export/json/route.ts
 * JSON data export endpoint for BI tools and custom integrations.
 *
 * GET /api/v1/export/json?collections=projects,tasks&tenantId=xxx&dateFrom=2024-01-01&dateTo=2024-12-31&fields=id,name,status&limit=5000&cursor=xxx&pretty=true&sanitizePII=true
 *
...
import { exportToJSON, validateCollections } from '@/lib/bi-export';
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Rate limit
    const rateKey = `export:json:${auth.apiKeyRecord?.keyPrefix || auth.tenantId || 'unknown'}`;
    const rateResult = await checkRateLimit(rateKey, EXPORT_RATE_LIMIT);
    if (!rateResult.allowed) {
      return NextResponse.json(
        { error: 'Rate limit excedido. Máximo 10 exportaciones JSON por minuto.' },
        {
          status: 429,
          headers: {
    ...
    console.error('[API /export/json] GET error:', message);
